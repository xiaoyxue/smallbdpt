...
void PathTracing::Render() {
	for (int y = 0; y < camera->film->heigh; ++y) {
		fprintf(stderr, "\rRendering %5.2f%%",
			(1.0 * y * camera->film->width) / camera->film->width / camera->film->heigh * 100.0);
#pragma omp parallel for schedule(dynamic, 1)
		for (int x = 0; x < camera->film->width; ++x) {
			...
			for (int s = 0; s < spp; ++s) {
				...
				r = r + Li(Ray(camera->o + d * 140, d), *sampler)*(1. / spp);

			}
			r = Vec3(clamp(r.x), clamp(r.y), clamp(r.z));
			camera->film->AddSample(Vec3(x, y), r);
		}
	}
...
//smallpt version
Vec3 PathTracing::radiance(const Ray &r, int depth, int E) {
	double t;                               // distance to intersection
	int id = 0;                               // id of intersected object
	Intersect isect;
	if (!intersect(r, t, id, isect)) return Vec3(); // if miss, return black
	...
	if (visual) {
		...
		if (flag) {
			camera->film->AddVisualPlane2(pFilm, Vec3(depth * 0.1, 1, 0));
		}
	}
	...
	if (obj.refl == DIFF) {                  // Ideal DIFFUSE reflection
		...
		for (int i = 0; i<numSpheres; i++) {
			...
			l.norm();
			if (intersect(Ray(x, l), t, id, isect) && id == i) {  // shadow ray
				double omega = 2 * PI*(1 - cos_a_max);
				e = e + f.mult(s.e*l.dot(nl)*omega)*INV_PI;  // 1/pi for brdf
			}
		}

		return obj.e*E + e + f.mult(radiance(Ray(x, d), depth, 0));
	}
	...
	return obj.e + f.mult(depth>0 ? (sampler->Get1D()<P ?   // Russian roulette
		...
		double nc = 1, nt = 1.5, nnt = into ? nc / nt : nt / nc, ddn = r.d.dot(nl), cos2t;
		if ((cos2t = 1 - nnt*nnt*(1 - ddn*ddn)) < 0)    // Total internal reflection
			return obj.e + f.mult(radiance(refRay, depth));
		Vec3 tdir = (r.d*nnt - n*((into ? 1 : -1)*(ddn*nnt + sqrt(cos2t)))).norm();
		...
		//std::cout <<" " <<c << std::endl;
		return obj.e + fr.mult(radiance(tRay, depth)) * std::abs(tDir.dot(n)) / pdf;
		//return obj.e + f.mult(depth>0 ? (sampler->Get1D()<P ?   // Russian roulette
...
Vec3 PathTracing::Li(const Ray &ray, Sampler &sampler) {
	...
	while (true) {
		...
		if (visual) {
			...
			Vec3 pFilm = camera->WordToScreen(isect.HitPoint, &flag);
			if (flag) {
				camera->film->AddVisualPlane(pFilm, false);
			}
		}


		if (bound == 0 || isDelta) {
			L = L + Throughput * spheres[id].e;
		}
		
		//direct lighting
		//if (!isect.bsdf->IsDelta() && bound != 0) {
		if (!isect.bsdf->IsDelta()){
			L += DirectIllumination(isect, Throughput, sampler);
		}
		++bound;
...
Vec3 DirectIllumination(const Intersect &isect, const Vec3 &Throughput, Sampler &sampler) {
	...
	if (!intersect(shadowRay, t, id, isec) || id != numSpheres - 1) return Vec3(0.0, 0.0, 0.0);
